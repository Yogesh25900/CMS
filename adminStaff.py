...
class crudstaff:
    def __init__(self,master,**kwargs):
        self.master = master
        self.master.title('Manage staff Details')
        self.master.geometry('900x700+0+2')
        self.master.resizable(False, False)
        file ="adminstaffbg.jpg"
        org =Image.open(file)
        ...
        self.register_frame = LabelFrame(self.master,text="Manage staff",font=(16),bd=7,relief=GROOVE,width=400,height=450)
        ...
        self.uname = Label(self.register_frame, text="staffname",font=(16))
        self.uname.place(x=2,y=180)
        self.e6 =Entry(self.register_frame, width=20,textvariable=self.staffname,font=(16))
        ...
        label_id = Label(self.register_frame, text="Enter staff Name",font=(16))
        ...
        search_button = Button(self.register_frame, text="Search",font=(16),command=self.search_staff)
        search_button.place(x=80,y=305)


        register_button = Button(self.register_frame, text="Add",font=(16),command=self.add_staff)  # Add a command to register new staffs
        register_button.place(x=150,y=305)

        update_button = Button(self.register_frame, text="Update",font=(16),command=self.update_staff)
        update_button.place(x=200,y=305)

        delete_button = Button(self.register_frame, text="Delete", font=(16),command=self.delete_staff)  # Add a command to delete staffs
        ...
        if  not name or not phone or not mail or not address or not uname or not pwd:
            ...
            if len(phone) != 10:
                ...
                try:
                    ...
                    query ="INSERT INTO tbstaff(name,phone,email,address,username,password) VALUES (%s,%s,%s,%s,%s,%s)"
    ...
    def update_staff(self):
        ...
        try:
            ...
            cursor = connection.cursor()
            cursor.execute("SELECT staffid from tbstaff WHERE LOWER(name) = LOWER(%s)",(c_id,))
            id = cursor.fetchone()[0]
            
            query ="UPDATE tbstaff SET name=%s,phone = %s ,email = %s , address = %s ,username = %s ,password = %s where staffid=%s "
            values =(name,phone,email,address,staffname,password,id)
    ...
    def delete_staff(self):
        ...
        try:
            ...
            query = "DELETE FROM tbstaff WHERE LOWER(name)=%s"
            values = (c_id,)
    ...
    def search_staff(self):
        ...
        try:
            ...
            if not c_id:
                ...
                query = "SELECT * FROM tbstaff WHERE staffid=%s or LOWER(name)= LOWER(%s)"
                values = (c_id,c_id)
                cursor.execute(query, values)
                ...
                if self.rows:
                    ...
                    staff_data = self.rows[0]  # Assuming you are fetching a single staff

                    # Update Entry widgets with the data from the database
                    
                    self.e2.delete(0, END)  # Clear the existing value
                    self.e2.insert(0, staff_data[1])  # Assuming the third column is rate
                    
                    self.e3.delete(0, END)  # Clear the existing value
                    self.e3.insert(0, staff_data[2])  # Assuming the third column is rate

                    self.e4.delete(0, END)  # Clear the existing value
                    self.e4.insert(0, staff_data[3])  # Assuming the third column is rate

                    self.e5.delete(0, END)  # Clear the existing value
                    self.e5.insert(0, staff_data[4])  # Assuming the third column is rate

                    self.e6.delete(0, END)  # Clear the existing value
                    self.e6.insert(0, staff_data[5])  # Assuming the third column is rate

                    self.e7.delete(0, END)  # Clear the existing value
                    self.e7.insert(0, staff_data[6])  # Assuming the third column is rate
