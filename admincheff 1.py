...
class crudchef:
    def __init__(self,master,**kwargs):
        self.master = master
        self.master.title('Manage chef Details')
        ...
        #assigning variable for chef table
        self.chefid = StringVar()
        self.cus_name =StringVar()
        self.mail =StringVar()
        self.phone =StringVar()
        self.address =StringVar()
        self.chefname =StringVar()
        self.key =StringVar()
         ...
         #new frame for registering chefs
        self.register_frame = LabelFrame(self.master,text="Manage chef",font=(16),bd=7,relief=GROOVE,width=400,height=450)
        ...
        self.uname = Label(self.register_frame, text="chefname",font=(16))
        self.uname.place(x=2,y=180)
        self.e6 =Entry(self.register_frame, width=20,textvariable=self.chefname,font=(16))
        ...
        label_id = Label(self.register_frame, text="Enter chef Name",font=(16))
        ...
        search_button = Button(self.register_frame, text="Search",font=(16),command=self.search_chef)
        search_button.place(x=80,y=305)


        register_button = Button(self.register_frame, text="Add",font=(16),command=self.add_chef)  # Add a command to register new chefs
        register_button.place(x=150,y=305)

        update_button = Button(self.register_frame, text="Update",font=(16),command=self.update_chef)
        update_button.place(x=200,y=305)

        delete_button = Button(self.register_frame, text="Delete", font=(16),command=self.delete_chef)  # Add a command to delete chefs
    ...
    def add_chef(self):
        ...
        c_id =self.chefid.get()
        name= self.cus_name.get()
        phone=self.phone.get()
        mail= self.mail.get()
        address= self.address.get()
        uname =self.chefname.get()
        pwd= self.key.get()
        if  not name or not phone or not mail or not address or not uname or not pwd:
            ...
            if len(phone) != 10:
                ...
                try:
                    ...
                    query ="INSERT INTO tbchef(name,phone,email,address,username,password) VALUES (%s,%s,%s,%s,%s,%s)"
                    values =(name,phone,mail,address,uname,pwd,)
                    cursor.execute(query,values)
                    connection.commit()
                    messagebox.showinfo("Success","chef registered Successfully")
    ...
    def update_chef(self):
        ...
        try:
            ...
            cursor = connection.cursor()
            cursor.execute("SELECT chefid from tbchef WHERE LOWER(name) = LOWER(%s)",(c_id,))
            id = cursor.fetchone()[0]
            
            query ="UPDATE tbchef SET name=%s,phone = %s ,email = %s , address = %s ,username = %s ,password = %s where chefid=%s "
            values =(name,phone,email,address,chefname,password,id)

            cursor.execute(query, values)
            connection.commit()
            messagebox.showinfo("Success", "chef details updated successfully!")
    ...
    def delete_chef(self):
        ...
        try:
            ...
            query = "DELETE FROM tbchef WHERE LOWER(name)=%s"
            values = (c_id,)

            cursor.execute(query, values)
            connection.commit()
            messagebox.showinfo("Success", "chef deleted successfully!")
    ...
    def search_chef(self):
        ...
        try:
            ...
            if not c_id:
                ...
                query = "SELECT * FROM tbchef WHERE chefid=%s or LOWER(name)= LOWER(%s)"
                values = (c_id,c_id)
                cursor.execute(query, values)
                ...
                if self.rows:
                    ...
                    chef_data = self.rows[0]  # Assuming you are fetching a single chef

                    # Update Entry widgets with the data from the database
                    
                    self.e2.delete(0, END)  # Clear the existing value
                    self.e2.insert(0, chef_data[1])  # Assuming the third column is rate
                    
                    self.e3.delete(0, END)  # Clear the existing value
                    self.e3.insert(0, chef_data[2])  # Assuming the third column is rate

                    self.e4.delete(0, END)  # Clear the existing value
                    self.e4.insert(0, chef_data[3])  # Assuming the third column is rate

                    self.e5.delete(0, END)  # Clear the existing value
                    self.e5.insert(0, chef_data[4])  # Assuming the third column is rate

                    self.e6.delete(0, END)  # Clear the existing value
                    self.e6.insert(0, chef_data[5])  # Assuming the third column is rate

                    self.e7.delete(0, END)  # Clear the existing value
                    self.e7.insert(0, chef_data[6])  # Assuming the third column is rate
